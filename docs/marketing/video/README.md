...
- **`FullAdVO`** (`src/vo/FullAdVO.tsx`) — тот же рекламный сюжет, но
  смонтированный под реальную озвучку ElevenLabs (`public/audio/real-vo.mp3`,
  не входит в репозиторий — положить свой файл сюда перед рендером). Тайминг
  13 бит (хаос → тревога → «Хватит!» → бренд → модули → было/стало → CTA)
  подобран под паузы в самой записи (`ffmpeg silencedetect`, ASR недоступен
  из песочницы) — при замене аудио на новую озвучку пересчитайте
  `beats[].frames` в `FullAdVO.tsx` под новые паузы/длину реплик.
- **Вставки без звука** (`src/bridge/*.tsx`) — короткие клипы для монтажа в
  свой ролик поверх собственной озвучки: `LogoBridge` (лого → дашборд, 3.5с),
  `PatientSearch` (поиск пациента, 4.3с), `TreatmentHistory` (карточка →
  история приёмов → документы, 6с).

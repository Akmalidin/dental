...
from apps.medicines.forms import PatientMedicineForm
from apps.tenancy import set_current_clinic, clear_current_clinic


class PatientMedicineFormClinicIsolationTestCase(TestCase):
    """Regression test: patient field must never leak patients from other clinics
    into the prescription form's dropdown (same class of bug as AppointmentForm)."""

    def setUp(self):
        self.clinic_a = Clinic.objects.create(name="Клиника A", slug="clinic-a-med")
        self.clinic_b = Clinic.objects.create(name="Клиника B", slug="clinic-b-med")
